...
def  main():
  ...
  installer = "\installer\SoreThroat Installer.exe"
   
  if demo:
    installer = "\installer\SoreThroat Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SoreThroat manual.pdf" 
  ]
  ...
  ZIPNAME = "SoreThroat-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SoreThroat-v" + FULLVERSIONSTR + "-win-demo.zip"
