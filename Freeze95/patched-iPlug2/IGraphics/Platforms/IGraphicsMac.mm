...
void* IGraphicsMac::OpenWindow(void* pParent)
{
  ...
  @try {
    ...
    SetScreenScale([[NSScreen mainScreen] backingScaleFactor] > 0.f ? [[NSScreen mainScreen] backingScaleFactor] : 1.f);
  ...
  @catch (NSException* exception) {
    DBGMSG("IGraphicsMac::OpenWindow - caught ObjC exception: %s\n", [[exception reason] UTF8String]);
    if (mView) {
      [(IGRAPHICS_VIEW*) mView killTimer];
      [(IGRAPHICS_VIEW*) mView removeFromSuperview];
      [(IGRAPHICS_VIEW*) mView release];
      mView = nullptr;
    }
    OnViewDestroyed();
  }
