...
Compilation options: -lang cpp -i -ct 1 -cn Freeze95DSP -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */
...
class Freeze95DSP : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-lang cpp -i -ct 1 -cn Freeze95DSP -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp32 = fTemp31 + fConst10 * (1.0f - std::floor(fConst11 * (fConst10 + (fTemp31 - fTemp28))));
			float fTemp33 = fTemp32 - fTemp28;
			int iTemp34 = int(fTemp33);
			int iTemp35 = std::min<int>(iConst9, std::max<int>(0, iTemp34 + 1));
			float fTemp36 = fTemp28 + std::floor(fTemp33);
			float fTemp37 = fTemp32 - fTemp36;
			float fTemp38 = fTemp36 + (1.0f - fTemp32);
			int iTemp39 = std::min<int>(iConst9, std::max<int>(0, iTemp34));
			float fTemp40 = 1.0f - fRec13[0];
			float fTemp41 = fTemp21 * (1.0f - (fTemp40 - std::floor(fTemp40)));
			float fTemp42 = fTemp30 + fConst10 * (1.0f - std::floor(fConst11 * (fConst10 + (fTemp30 - fTemp41))));
			float fTemp43 = fTemp42 - fTemp41;
			int iTemp44 = int(fTemp43);
			int iTemp45 = std::min<int>(iConst9, std::max<int>(0, iTemp44 + 1));
			float fTemp46 = fTemp41 + std::floor(fTemp43);
			float fTemp47 = fTemp42 - fTemp46;
			float fTemp48 = fTemp46 + (1.0f - fTemp42);
			int iTemp49 = std::min<int>(iConst9, std::max<int>(0, iTemp44));
			float fTemp50 = ((iTemp17) ? fTemp29 * (fVec0[(IOTA0 - iTemp49) & 1048575] * fTemp48 + fTemp47 * fVec0[(IOTA0 - iTemp45) & 1048575]) : (fVec0[(IOTA0 - iTemp39) & 1048575] * fTemp38 + fTemp37 * fVec0[(IOTA0 - iTemp35) & 1048575]) * fTemp29) * fTemp15;
			float fTemp51 = 6.0f * fRec1[0] + 1.0f;
			fRec2[0] = ((iTemp4) ? fTemp50 * fTemp51 / (6.0f * fRec1[0] * std::fabs(fTemp50) + 1.0f) : fRec2[1]);
			float fTemp52 = std::floor(std::max<float>(-1.0f, std::min<float>(1.0f, fRec2[0])) * fTemp3 + 0.5f) / fTemp3;
			fVec4[0] = fTemp52;
			float fTemp53 = 1.0f - fTemp1;
			fRec0[0] = -((fRec0[1] * fTemp53 - (fTemp52 + fVec4[1])) / fTemp2);
			float fTemp54 = Freeze95DSP_faustpower2_f(fRec0[0]);
			fRec17[0] = fSlow22 + fConst1 * fRec17[1];
			output0[i0] = FAUSTFLOAT(fTemp0 + fRec17[0] * (fRec0[0] * (fTemp54 + 27.0f) / (9.0f * fTemp54 + 27.0f) - fTemp0));
			float fTemp55 = float(input1[i0]);
			fVec5[IOTA0 & 1048575] = fTemp55;
			float fTemp56 = ((iTemp17) ? fTemp29 * (fTemp48 * fVec5[(IOTA0 - iTemp49) & 1048575] + fTemp47 * fVec5[(IOTA0 - iTemp45) & 1048575]) : fTemp29 * (fTemp38 * fVec5[(IOTA0 - iTemp39) & 1048575] + fTemp37 * fVec5[(IOTA0 - iTemp35) & 1048575]));
			fRec19[0] = ((iTemp4) ? fTemp15 * fTemp51 * fTemp56 / (6.0f * fRec1[0] * std::fabs(fTemp15 * fTemp56) + 1.0f) : fRec19[1]);
			float fTemp57 = std::floor(std::max<float>(-1.0f, std::min<float>(1.0f, fRec19[0])) * fTemp3 + 0.5f) / fTemp3;
			fVec6[0] = fTemp57;
			fRec18[0] = -((fTemp53 * fRec18[1] - (fTemp57 + fVec6[1])) / fTemp2);
			float fTemp58 = Freeze95DSP_faustpower2_f(fRec18[0]);
			output1[i0] = FAUSTFLOAT(fTemp55 + fRec17[0] * (fRec18[0] * (fTemp58 + 27.0f) / (9.0f * fTemp58 + 27.0f) - fTemp55));
			IOTA0 = IOTA0 + 1;
			iVec1[1] = iVec1[0];
			fRec1[1] = fRec1[0];
			iRec3[1] = iRec3[0];
