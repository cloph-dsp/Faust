...
def scale_entry(name, n, ivals):
    elems = ", ".join(str(x) for x in ivals)
    # NO trailing spaces — use explicit no-space format
    return '    { "%s", %2d, {%s}} },' % (name, n, elems)

def chord_entry(name, n, ivals):
    elems = ", ".join(str(x) for x in ivals)
    if name == "":
        name_part = '""'
    else:
        name_part = '"%s"' % name
    return '    { %s, %2d, {%s}} },' % (name_part, n, elems)

# Generate
lines = []
lines.append("// MUSIC-THEORY DATABASE")
lines.append("// ScaleType: see 3BeatProblem.h (struct defined there).")
lines.append("// kScales[80] - canonical + exotic scales")
lines.append("const ScaleType kScales[80] = {")
lines.append("  // 0-24: Common scales / modes")
for i, (name, n, ivals) in enumerate(scales):
    if i == 25:
        lines.append("  // 25-79: Exotic / jazz / world scales")
    lines.append(scale_entry(name, n, ivals))
lines.append("};")
lines.append("")
lines.append("// kChordTypes[100] - common + extended + pad chords")
lines.append("const ChordType kChordTypes[100] = {")
lines.append("  // 0-34: Common chord types")
chord_sections = {35: "  // 35-49: Extended / altered", 50: "  // 50-64: Added-tone / suspended", 65: "  // 65-99: Pad placeholders"}
for i, (name, n, ivals) in enumerate(chords):
    if i in chord_sections:
        lines.append(chord_sections[i])
    lines.append(chord_entry(name, n, ivals))
lines.append("};")
...
# Find insertion point: after #include <cstring>, before constructor
insert_idx = None
...
if insert_idx is None:
    raise ValueError("Could not find #include <cstring>")

# Build new content
new_content = orig[:insert_idx] + ['\n'] + [l + '\n' for l in lines] + orig[insert_idx:]

with open(r'D:\Github\VST3 Developments\3BeatProblem\3BeatProblem.cpp', 'w') as f:
    f.writelines(new_content)

print(f"Inserted {len(lines)} lines at index {insert_idx}")
